...
握手可以分为两个部分，第一部分是客户端和服务器之间协商建立其安全通信的过程，第二部分是客户端与服务器之间就协商建立好安全通信进行测试的过程。所以。。。这里我们先讲第一个部分。
...
当服务器收到客户端的Client Hello信息之后，服务器就要忙起来了，在下面服务器告诉客户端（1）我们就使用什么TLS版本，什么密码套件达成一致；（2）向客户端证明我是谁；（2）我们来传递密钥交换的必要参数。
